...
describe("ControlRail", () => {
  ...
  it("renders settings cluster on desktop and generate button", () => {
    ...
    expect(wrapper.get('[data-testid="control-rail"]').exists()).toBe(true);
    expect(wrapper.get('[data-testid="settings-cluster"]').exists()).toBe(true);
    expect(wrapper.get('[data-testid="generate-button"]').exists()).toBe(true);
  });
