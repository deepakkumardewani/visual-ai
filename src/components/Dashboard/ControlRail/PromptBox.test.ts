...
describe("PromptBox", () => {
  ...
  it("renders custom textarea without v-textarea", () => {
    ...
    expect(wrapper.get('[data-testid="prompt-box"]').exists()).toBe(true);
    expect(wrapper.get('[data-testid="prompt-textarea"]').exists()).toBe(true);
  ...
  it("restores promptText on mount", async () => {
    ...
    expect(wrapper.get('[data-testid="prompt-textarea"]').element.value).toBe("Saved prompt");
  });
