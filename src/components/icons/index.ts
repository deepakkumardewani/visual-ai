import IconCamera from '@/components/icons/IconCamera.vue'
import IconCameraDark from '@/components/icons/IconCameraDark.vue'
import IconCoin from '@/components/icons/IconCoin.vue'
import IconConnection from '@/components/icons/IconConnection.vue'
import IconConnectionDark from '@/components/icons/IconConnectionDark.vue'
import IconDropper from '@/components/icons/IconDropper.vue'
import IconDropperDark from '@/components/icons/IconDropperDark.vue'
import IconExpand from '@/components/icons/IconExpand.vue'
import IconExpandDark from '@/components/icons/IconExpandDark.vue'
import IconFluxFast from '@/components/icons/IconFluxFast.vue'
import IconFluxFastDark from '@/components/icons/IconFluxFastDark.vue'
import IconFluxPro from '@/components/icons/IconFluxPro.vue'
import IconFluxProDark from '@/components/icons/IconFluxProDark.vue'
import IconFluxRealism from '@/components/icons/IconFluxRealism.vue'
import IconFluxRealismDark from '@/components/icons/IconFluxRealismDark.vue'
import IconImageFrame from '@/components/icons/IconImageFrame.vue'
import IconImageFrameDark from '@/components/icons/IconImageFrameDark.vue'
import IconLayers from '@/components/icons/IconLayers.vue'
import IconLayersDark from '@/components/icons/IconLayersDark.vue'
import IconStar from '@/components/icons/IconStar.vue'
import IconUsers from '@/components/icons/IconUsers.vue'
import IconUsersDark from '@/components/icons/IconUsersDark.vue'
import IconCinematic from '@/components/icons/sizes/IconCinematic.vue'
import IconCinematicDark from '@/components/icons/sizes/IconCinematicDark.vue'
import IconPortrait from '@/components/icons/sizes/IconPortrait.vue'
import IconPortraitDark from '@/components/icons/sizes/IconPortraitDark.vue'
import IconSocialPost from '@/components/icons/sizes/IconSocialPost.vue'
import IconSocialPostDark from '@/components/icons/sizes/IconSocialPostDark.vue'
import IconSocialStory from '@/components/icons/sizes/IconSocialStory.vue'
import IconSocialStoryDark from '@/components/icons/sizes/IconSocialStoryDark.vue'
import IconSquare from '@/components/icons/sizes/IconSquare.vue'
import IconSquareDark from '@/components/icons/sizes/IconSquareDark.vue'
import IconStandard from '@/components/icons/sizes/IconStandard.vue'
import IconStandardDark from '@/components/icons/sizes/IconStandardDark.vue'
import IconWidescreen from '@/components/icons/sizes/IconWidescreen.vue'
import IconWidescreenDark from '@/components/icons/sizes/IconWidescreenDark.vue'

export default {
  // Normal icons
  camera: IconCamera,
  expand: IconExpand,
  dropper: IconDropper,
  imageFrame: IconImageFrame,
  coin: IconCoin,
  star: IconStar,
  fluxFast: IconFluxFast,
  fluxRealism: IconFluxRealism,
  fluxPro: IconFluxPro,
  layers: IconLayers,
  connection: IconConnection,
  users: IconUsers,
  // Size icons - normal
  square: IconSquare,
  portrait: IconPortrait,
  standard: IconStandard,
  socialPost: IconSocialPost,
  socialStory: IconSocialStory,
  widescreen: IconWidescreen,
  cinematic: IconCinematic,

  // Dark variants
  cameraDark: IconCameraDark,
  expandDark: IconExpandDark,
  dropperDark: IconDropperDark,
  imageFrameDark: IconImageFrameDark,
  fluxFastDark: IconFluxFastDark,
  fluxRealismDark: IconFluxRealismDark,
  fluxProDark: IconFluxProDark,
  layersDark: IconLayersDark,
  connectionDark: IconConnectionDark,
  usersDark: IconUsersDark,
  // Size icons - dark variants
  squareDark: IconSquareDark,
  portraitDark: IconPortraitDark,
  standardDark: IconStandardDark,
  socialPostDark: IconSocialPostDark,
  socialStoryDark: IconSocialStoryDark,
  widescreenDark: IconWidescreenDark,
  cinematicDark: IconCinematicDark
}
